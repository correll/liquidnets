...
class SimpleLiquidPolicy(torch.nn.Module):
    """Match the retrained policy architecture."""
    def __init__(self, state_dim=57, hidden_dim=291, action_dim=7, pred_horizon=16):
        ...
        self.encoder = torch.nn.Sequential(
            torch.nn.Linear(state_dim * pred_horizon, hidden_dim),
            torch.nn.ReLU(),
            torch.nn.Linear(hidden_dim, hidden_dim),
            torch.nn.ReLU(),
        )
        
        self.head = torch.nn.Linear(hidden_dim, action_dim)
    
    def forward(self, state_history):
        B = state_history.shape[0]
        x = state_history.reshape(B, -1)
        x = self.encoder(x)
        x = self.head(x)
        return torch.tanh(x)
...
def get_obs_from_env(obs):
    ...
    if "object-state" in obs:
        return np.concatenate([
            ...
            obs["robot0_eef_quat_site"].astype(np.float32),
            obs["robot0_gripper_qpos"].astype(np.float32),
            obs["robot0_gripper_qvel"].astype(np.float32),
            obs["robot0_joint_pos"].astype(np.float32),
            obs["robot0_joint_pos_cos"].astype(np.float32),
            obs["robot0_joint_pos_sin"].astype(np.float32),
            obs["robot0_joint_vel"].astype(np.float32),
        ], axis=-1)
    raise KeyError(f"Unsupported observation keys: {sorted(list(obs.keys()))}")
...
def evaluate_policy(policy, num_episodes=10, device='cpu', stats=None, policy_name=""):
    ...
    for ep_idx in range(num_episodes):
        ...
        initial_obj_pos = obs['object'].copy() if 'object' in obs else obs['Can_pos'].copy()
        total_reward = 0
        ...
        # Get final metrics
        final_obj_pos = obs['object'].copy() if 'object' in obs else obs['Can_pos'].copy()
...
def main():
    ...
    checkpoint = torch.load(args.retrained_ckpt, map_location=device, weights_only=False)
    ...
    try:
        original_checkpoint = torch.load(args.original_ckpt, map_location=device, weights_only=False)
