...
\begin{table*}[t]
    ...
    \begin{tabular}{llrrrr}
        ...
        \midrule
        RoboMimic Can & Liquid + MDN & 0 & -- & \textbf{0.0001} & \textbf{0.3257} \\
        RoboMimic Can & Diffusion & 0 & -- & 0.0000 & 2.1633 \\
        \bottomrule
    \end{tabular}
\end{table*}
...
\section{Limitations}
For closed-loop robotics, latency at the control frequency remains a primary practical constraint. Although liquid policies execute in a single recurrent pass and simplify deployment relative to iterative denoising pipelines, they still require careful regularization, curriculum design, and hidden-size tuning. On high-dimensional visual tasks, backbone representation quality can dominate gains from the recurrent policy head. Closed-loop comparisons also remain task-dependent: on Push-T, liquid primarily improves latency while achieving success rates comparable to diffusion; on PointMaze, the liquid model improves task-completion metrics; and on RoboMimic Can, both policies achieve 0\% closed-loop task success due to the offline-to-online deployment gap, though liquid retains substantially lower action-tracking error ($0.3257$ vs.\ $2.1633$ mean $\ell_2$). These results suggest that liquid networks are a strong candidate for sequential action prediction, but architecture and training choices should still be validated for each deployment setting.

A critical observation: despite achieving $2.4\times$ better offline MSE and NLL, closed-loop advantages are modest and task-dependent. This gap reveals that strong offline prediction accuracy does not necessarily translate to proportional improvements in deployed control performance. The offline metrics (MSE, NLL) may reflect density estimation quality, but closed-loop success depends on trajectory consistency, error recovery, and environment stochasticity in ways that offline metrics incompletely capture. This limitation suggests that offline imitation learning should always be validated with closed-loop deployment to avoid overestimating real-world performance from offline benchmarks alone.

\paragraph{Limited evaluation scope.}
This work evaluates liquid networks on relatively simple simulated tasks (Push-T contact manipulation, PointMaze navigation) where modest datasets suffice. These domains do not require the rich visual reasoning or long-horizon planning necessary for complex real-world manipulation. More challenging tasks such as large-scale RoboMimic Can manipulation (where both liquid and diffusion achieve 0\% closed-loop success due to the offline-to-online gap) would likely require large-scale pretraining on diverse demonstration corpora, additional architectural components (e.g., vision transformers, hierarchical action abstractions), or hybrid RL fine-tuning to achieve practical deployment success. Future work should explore whether liquid networks maintain their efficiency and accuracy advantages when paired with such augmentations on genuinely complex tasks.
