...
For RoboMimic Can, we evaluate the released checkpoints under the same deployed policy architecture and report closed-loop action-tracking error as mean per-episode $\ell_2$ in original action units. Liquid + MDN achieves $0.3257 \pm 0.0492$ while diffusion achieves $2.1633 \pm 0.0389$, indicating substantially better control quality for liquid in this case study.

\begin{table*}[t]
    \centering
    \small
    \caption{Closed-loop evaluation summary across all three case studies. For Push-T and PointMaze, higher is better for success, distance-success, and reward. For RoboMimic Can, lower is better for mean per-episode action $\ell_2$. Bold entries indicate the better model per metric within each task. Push-T uses a matched 100-episode comparison; PointMaze reports 50-trial aggregate means. For PointMaze, \emph{Success} uses the environment success flag, while \emph{Distance-Success} uses a stricter rollout minimum-distance threshold of $0.20$.}
    ...
    \begin{tabular}{llrrrr}
        ...
        \midrule
        RoboMimic Can & Liquid + MDN & -- & -- & -- & \textbf{0.3257} \\
        RoboMimic Can & Diffusion & -- & -- & -- & 2.1633 \\
        \bottomrule
    \end{tabular}
\end{table*}
